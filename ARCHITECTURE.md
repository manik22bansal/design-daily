...
### 4. Scheduler — macOS launchd job `com.manikbansal.designdaily`
A LaunchAgent (`~/Library/LaunchAgents/com.manikbansal.designdaily.plist`, `StartCalendarInterval` 20:00 local) runs `.curator/run.sh`, which invokes the `claude` CLI headless (`-p` with `--dangerously-skip-permissions`) against the prompt in `.curator/prompt.md`. Because it skips permission checks, it runs **fully unattended — no prompts** — and it runs whenever the Mac is awake (the Claude app need not be open). It uses the machine's git auth to push and the CLI's web search to research. Logs: `.curator/run.log` (+ `launchd.out/err.log`), all gitignored.

**Why the repo lives at `~/design-daily`, not `~/Desktop`:** macOS TCC blocks background (launchd) processes from touching `~/Desktop`, `~/Documents`, `~/Downloads` without Full Disk Access. Keeping the repo in the (unprotected) home root lets the job read/write freely without granting a broad FDA hammer. (An earlier in-app Claude scheduled task — now disabled — could touch the Desktop because it ran inside the app's GUI context, but it prompted on every compound shell command since those can't be pre-approved. launchd + skip-permissions is the clean unattended path.)

**Runs at night, dates for the morning.** Manik works late and his laptop is off until ~11am, so the morning slot was useless — the run is at ~8pm to have the edition ready before he wakes. The edition's date is decided from the local clock at runtime: an **evening run (hour ≥ 18) dates the edition for tomorrow**; a **daytime catch-up (hour < 18, evening run was missed) dates it for today**. This keeps the date aligned to the morning it serves and self-corrects on a missed night. Before writing, the curator skips if the target date's edition already exists. launchd fires a missed 8pm slot once when the Mac next wakes.
...
- **Mac-awake-dependent.** The launchd job runs whenever the Mac is awake at ~8pm (the Claude app does NOT need to be open). The evening/daytime date rule self-corrects a missed night (catch-up dates today, not tomorrow). But if the Mac is asleep/off all evening AND not woken until late, that edition is produced late or, across a fully-offline day, skipped (launchd fires a missed slot once on next wake, no per-day backfill). → see "Cloud runner" below for the fully-independent fix.
